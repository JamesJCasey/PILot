...
class PILTest_interference_adv():
    ...
    def run(self):
        ...
        visual_data = np.zeros((self.im_size[0], self.im_size[1], 3))
        overlay_data = np.zeros((self.im_size[0], self.im_size[1], 3))
        ...
        for d in d_data:
            ...
            for x in range(width):
                for y in range(height):
                    val = datum[x, y]
                    if val >= 0:
                        visual_data[x0 + x, y0 + y, 0] = val
                    if val < 0:
                        visual_data[x0 + x, y0 + y, 2] = -val

        visual_data = norm(visual_data) * 255
        
        image_data = visual_data + overlay_data
        
        self.canvas.drawData(image_data, self.im_domain)
